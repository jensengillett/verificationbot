...
def backup_databases(always_run=True):
    ...
    zip_file = zipfile.ZipFile(loczip, "w", zipfile.ZIP_DEFLATED)

    only_files = [f for f in listdir(data_path) if isfile(join(data_path, f))]
    for file in only_files:
        backup_file(zip_file, f"{data_path}{file}")

    logging.info(f"Backed up {len(only_files)} files to {backups_folder_name}/{subfolder_name}/{zip_name}")
...
def backup_file(zip_file, filename: str):
